...
# Build the project (debug mode) — type-checks the Zig FFI layer
build *args:
...
clean:
    @echo "Cleaning..."
    rm -rf src/interface/ffi/zig-out src/interface/ffi/.zig-cache docs/generated docs/man
...
# Run all tests (Zig unit tests in src/main.zig, then the FFI integration stub)
test *args:
    @echo "Running tests..."
    cd src/interface/ffi && zig test src/main.zig {{args}}
    cd src/interface/ffi && zig test test/integration_test.zig {{args}}
    @echo "Tests passed!"

# Run tests with verbose reference-trace output
test-verbose:
    @echo "Running tests (verbose)..."
    cd src/interface/ffi && zig test src/main.zig -freference-trace=16
    cd src/interface/ffi && zig test test/integration_test.zig -freference-trace=16

# Smoke test — the FFI layer compiles cleanly under zig fmt --check
test-smoke:
    @echo "Smoke test..."
...
# NOTE: `bench` and `readiness` recipes were removed rather than left as
# `echo "TODO"` stubs — benches/ has no real benchmark for this repo yet, and
# there is no automated readiness test suite (docs/status/READINESS.adoc is a
# hand-maintained status doc, not a test harness). Add them back once real
# work exists to back them.
...
# Run the full merge-requirement test suite (ALL categories currently backed
# by real commands — see the NOTE above `bench`/`readiness` for what's missing)
...
# Run linter — typechecks the Idris2 ABI package (skips gracefully if idris2
# is not installed locally; a real ABI compile error still fails the recipe)
lint:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    if command -v idris2 >/dev/null 2>&1; then
        ...
        echo "SKIP: idris2 not installed — cannot typecheck abi.ipkg"
    fi
...
# NOTE: `run`, `run-verbose`, and `install` recipes were removed rather than
# left as `echo "Run not configured yet"` stubs. build.zig does not currently
# define an executable or library artifact (see src/interface/ffi/build.zig)
# — there is nothing to run or install yet. Add these back once build.zig
# grows a real target.
...
deps:
    ...
    command -v zig >/dev/null 2>&1 || { echo "MISSING: zig"; exit 1; }
    zig version
    if command -v idris2 >/dev/null 2>&1; then
        idris2 --version
    else
        echo "MISSING (optional): idris2 — needed for 'just lint'"
    fi
