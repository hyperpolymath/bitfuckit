...
for OWNER in "${OWNERS[@]}"; do
    ...
    if [[ -z "$GITHUB_REPOS" ]]; then
        echo "ERROR: No repositories found on GitHub or API error for ${OWNER}. Aborting to prevent mass deletion (fail-closed)." >&2
        exit 1
    fi

    echo "=> Found $(echo "$GITHUB_REPOS" | wc -w) repositories on GitHub for ${OWNER}."
        ...
        if [[ -n "$orphans" ]]; then
            for orphan in $orphans; do
                ...
                if [[ "$PRUNE_CONFIRMED" == "true" ]]; then
                    echo "         -> Pruning ${orphan} on ${forge}... (Dry run)"
                fi
            done
        else
...
echo "=> Sweep complete for all targets."
