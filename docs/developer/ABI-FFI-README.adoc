...
# bitfuckit ABI/FFI Documentation
...
│       └── include/
│           └── bitfuckit.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── bitfuckit.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/bitfuckit.h
```
...
int main() {
    void* handle = bitfuckit_init();
    if (!handle) return 1;

    int result = bitfuckit_process(handle, 42);
    if (result != 0) {
        const char* err = bitfuckit_last_error();
...
Compile with:
```bash
gcc -o example example.c -lbitfuckit -L./zig-out/lib
```

### From Idris2

```idris
import bitfuckit.ABI.Foreign
...
#[link(name = "bitfuckit")]
extern "C" {
    fn bitfuckit_init() -> *mut std::ffi::c_void;
    fn bitfuckit_free(handle: *mut std::ffi::c_void);
    fn bitfuckit_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = bitfuckit_init();
        assert!(!handle.is_null());

        let result = bitfuckit_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:bitfuckit_init, libbitfuckit), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:bitfuckit_process, libbitfuckit), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:bitfuckit_free, libbitfuckit), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/bitfuckit.h
   ```
